  ...
  return (
    <div className="space-y-3">
      <div className="text-center">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Policy auswählen</h2>
        <p className="text-xs text-gray-500">YAML-Datei mit Compliance-Regeln</p>
      </div>
        ...
        <div className={`border rounded p-3 text-center bg-white dark:bg-gray-800 ${isComplete ? 'border-green-500' : 'border-gray-200 dark:border-gray-700'}`}>
          {!isComplete ? (
            <>
              ...
              <p className="text-xs text-gray-500 mb-2">Compliance-Checks definieren</p>
              <button
              ...
              >
                {isLoading ? 'Lade...' : 'Policy auswählen'}
              </button>
            </>
          ) : (
            <>
              <div className="flex items-center justify-center mb-2">
                ...
                <span className="ml-1 text-xs font-semibold text-green-600">Policy geladen</span>
              </div>
              <div className="space-y-1 text-left bg-gray-50 dark:bg-gray-900 rounded p-2 text-[10px]">
                ...
                <div className="flex justify-between"><span className="text-gray-500">Regeln:</span><span>{policyInfo.rules_count}</span></div>
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={handleShowDetails} disabled={loadingContent} className="flex-1 px-2 py-1 text-[10px] text-blue-600 bg-blue-50 rounded hover:bg-blue-100">
                  {loadingContent ? 'Lade...' : showDetails ? 'Ausblenden' : 'Details'}
                </button>
                <button onClick={handleLoadPolicy} disabled={isLoading} className="flex-1 px-2 py-1 text-[10px] text-gray-600 bg-gray-100 rounded hover:bg-gray-200">Andere</button>
              </div>
              {showDetails && (
