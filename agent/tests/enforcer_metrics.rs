...
use cap_agent::orchestrator::{Enforcer, EnforceOptions, OrchestratorContext, DriftTracker, VerdictPair, Verdict};
...
fn test_metrics_increment_shadow_only() {
    ...
    let policy_id = format!("test_shadow_only_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    assert_eq!(shadow_after, shadow_before + 1, "Shadow counter should increment");
    assert_eq!(enforced_after, enforced_before, "Enforced counter should not increment in shadow-only mode");
}
...
fn test_metrics_increment_enforced() {
    ...
    // Use unique policy_id to avoid metric sharing across tests
    let policy_id = format!("test_enforced_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    assert_eq!(shadow_after, shadow_before + 1, "Shadow counter should increment (always computed)");
    assert_eq!(enforced_after, enforced_before + 1, "Enforced counter should increment");
...
fn test_drift_event_increments_counter() {
    ...
    let policy_id = format!("test_drift_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    assert_eq!(drift_after, drift_before + 1, "Drift counter should increment");
}
...
fn test_drift_ratio_updates() {
    ...
    let policy_id = format!("test_ratio_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
...
fn test_selection_latency_histogram() {
    ...
    assert!(samples_after >= samples_before + 3, "Histogram should have at least 3 new observations");
}
...
fn test_drift_tracker_threshold() {
    ...
    let mut tracker = DriftTracker::new();
    let policy_id = format!("test_threshold_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    assert!(!tracker.exceeds_threshold(threshold), "Should not exceed threshold at exactly 0.5%");
    ...
    assert!(tracker.exceeds_threshold(threshold), "Should exceed threshold above 0.5%");
}

#[test]
fn test_no_pii_in_metrics() {
    // Test: Metriken enthalten keine PII (nur policy_id und mode als Labels)
    let policy_id = format!("test_pii_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
