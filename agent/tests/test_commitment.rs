...
fn test_company_root_combines_roots() {
    ...
    assert!(
        company_root.starts_with("0x"),
        "Company root should start with 0x"
    );
    assert_ne!(
        company_root, supplier_root,
        "Company root should differ from supplier root"
    );
    assert_ne!(
        company_root, ubo_root,
        "Company root should differ from UBO root"
    );
}
...
fn test_ubo_root_computation() {
    ...
    assert!(
        root.len() > 10,
        "UBO root should be longer than 10 characters"
    );
}
...
fn test_empty_supplier_list() {
    ...
    assert!(
        root.starts_with("0x"),
        "Empty supplier root should start with 0x"
    );
}
...
fn test_empty_ubo_list() {
    ...
    assert!(
        root.starts_with("0x"),
        "Empty UBO root should start with 0x"
    );
}
...
fn test_different_suppliers_produce_different_roots() {
    ...
    assert_ne!(
        root1, root2,
        "Different suppliers should produce different roots"
    );
}
