...
use cap_agent::audit::{AuditChain, AuditEventResult, verify_chain, export_events};
...
fn test_full_workflow_append_verify_export() {
    ...
    for i in 0..10 {
        chain.append(
            ...
            Some(if i % 2 == 0 { AuditEventResult::Ok } else { AuditEventResult::Warn }),
            Some(format!("run-{}", i / 3)),
        ).unwrap();
    }
...
fn test_dod_verify_10k_events_under_3s() {
    ...
    println!("✅ DoD Performance Criterion MET: 10,000 events verified in {:?} (≤ 3s)", verify_duration);
}
...
fn test_dod_append_p95_under_0_5ms() {
    ...
    println!("✅ DoD Performance Criterion MET: Append p95 = {:?} (≤ 0.5ms)", p95);
}
...
fn test_export_with_complex_filters() {
    ...
    chain.append("event_4".to_string(), Some("lksg.v1".to_string()), None, None, None, None).unwrap();
