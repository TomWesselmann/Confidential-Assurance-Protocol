...
fn create_test_bundle(
    ...
    for (i, unit) in proof_units.iter().enumerate() {
        ...
        let manifest_file_hash =
            cap_agent::crypto::hex_lower_prefixed32(cap_agent::crypto::sha3_256(&manifest_bytes));
        ...
        let proof_hash =
            cap_agent::crypto::hex_lower_prefixed32(cap_agent::crypto::sha3_256(&proof_bytes));
...
fn create_test_manifest(path: &str, policy_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let manifest = json!({
...
fn test_verify_bundle_v1_single_unit_ok() {
    ...
    assert!(
        stdout.contains("cap-bundle.v1"),
        "Should detect bundle.v1 format"
    );
    assert!(
        stdout.contains("Bundle-Verifikation abgeschlossen"),
...
fn test_verify_bundle_legacy_fallback() {
    ...
    create_test_manifest(&manifest_path, "legacy.policy").expect("Failed to create manifest");
    ...
    let manifest_hash = cap_agent::crypto::hex_lower_prefixed32(cap_agent::crypto::sha3_256(
    ...
    assert!(
        stdout.contains("Legacy") || stdout.contains("pre-bundle.v1"),
...
fn test_verify_bundle_multiple_units() {
    ...
    let units = vec![
        ...
        TestProofUnit {
            ...
            policy_hash: "0xdef1234567890123456789012345678901234567890123456789012345678901"
                .to_string(),
            depends_on: vec![],
        },
    ];
...
fn test_verify_bundle_circular_dependency() {
    ...
    let units = vec![
        ...
        TestProofUnit {
            ...
            policy_hash: "0x1231234567890123456789012345678901234567890123456789012345678901"
                .to_string(),
...
fn test_verify_bundle_missing_proof_file() {
    ...
    // Delete proof file
    fs::remove_file(format!("{}/proof_0.capz", bundle_dir)).expect("Failed to remove proof file");
