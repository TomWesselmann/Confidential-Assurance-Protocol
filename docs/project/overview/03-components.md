...
> Diese Dokumentation beschreibt die Komponenten der Minimal Local Agent Version.
> Entfernte Enterprise-Features sind entsprechend markiert:
...
| **Key Providers** | 1 | Schlüssel-Speicher | Nur SoftwareProvider (Ed25519 dateibasiert) |
| **Support** | 6 | Hilfssysteme | Logging, Metrics |
| **Desktop App** | 10+ | Standalone-Software | Tauri 2.0 Offline Desktop Proofer mit integrierter WebUI (v0.12.2) |
| **Monitoring & Observability** | 8 | Überwachungszentrale | Production Monitoring Stack |
...
struct VerifyRequest {
    ...
    backend: String,              // "mock" (nur mock in Minimal Local Agent)
...
struct Proof {
    ...
    type: String,                       // "mock" (nur mock in Minimal Local Agent)
    statement: String,
    manifest_hash: String,
    policy_hash: String,
...
**Implementierungen:**
- `MockZK` - Mock backend (einziges verfügbares Backend)
...
**Logic:**
```
// Vereinfacht - nur mock verfügbar
→ immer "mock" (einziges Backend)
```
...
struct CapzHeader {
    magic: [u8; 4],             // b"CAPZ"
    version: u16,               // 0x0002
    backend: u8,                // 0=mock (nur mock in Minimal Local Agent)
    reserved: u8,
    vk_hash: [u8; 32],
    params_hash: [u8; 32],
    payload_len: u32,
...
struct ProofUnit {
    ...
    pub backend: String,         // "mock" (nur mock in Minimal Local Agent)
}
```
...
## 16. Desktop App Layer (Tauri 2.0) - v0.12.2

**Management-Zusammenfassung:** Native Desktop-Anwendung für komplett offline Compliance-Workflow. Wie eine eigenständige Software (Word, Excel) - keine Server, keine Cloud, alle Daten bleiben auf dem lokalen Rechner. Enthält die integrierte WebUI (React-basiert).
...
### tauri-frontend/src/store/workflowStore.ts
...
### tauri-frontend/src/components/workflow/WorkflowStepper.tsx
...
### tauri-frontend/src/components/layout/ProjectSidebar.tsx
...
### tauri-frontend/src/components/audit/AuditTimeline.tsx
...
### tauri-frontend/src/lib/tauri.ts
...
- **90+ Module** für verschiedene Funktionen (Backend + Desktop App + Monitoring)
...
- **User-Friendly** durch integrierte React-WebUI in Tauri Desktop App für nicht-technische Nutzer
- **Offline-fähig** durch Tauri Desktop App ohne Server-Abhängigkeit

**Aktuelle Version (v0.12.2):**
- 🖥️ **Desktop App** (10+ Komponenten) - Tauri 2.0 mit integrierter React-WebUI
...
**Entfernt in v0.12.0:**
- ❌ **Standalone Web UI** - Ersetzt durch Tauri-integrierte WebUI
- ❌ **Axios HTTP Client** - Ersetzt durch Tauri IPC

**Aus früheren Versionen (noch verfügbar):**
...
- 📤 **File Upload** - Multipart Upload Handler für Proof Packages (via Verifier API)
