...
**Gesamt: 212+ Begriffe** (Stand: v0.12.2 – Minimal Local Agent)

> **📌 Hinweis Minimal Local Agent (v0.12.2):**
> Diese Version konzentriert sich auf lokale, dateibasierte Schlüsselverwaltung mit Ed25519.
> Folgende Enterprise-Features wurden entfernt:
> - **PKCS#11/HSM** – Hardware Security Module Integration
> - **CloudKMS** – Google Cloud Key Management Service
> - **zkvm/halo2** – Zero-Knowledge Proof Backends (nur Mock-Backend verfügbar)
> - **Sanctions Lists** – OFAC/EU/UN Sanktionslisten-Integration
> - **Standalone WebUI** – HTTP-basierte Web-Oberfläche (nur Tauri Desktop App)
...
> **📌 Minimal Local Agent:** Sanktionslisten-Integration wurde in v0.12.0 entfernt. Das System bietet weiterhin Policy-basierte Compliance-Prüfungen.
...
**Im Projekt (Minimal Local Agent):** **Einziger unterstützter Signaturalgorithmus.** Verwendet für:
- Manifest-Signaturen
- Registry-Entry-Signaturen
- Key-Attestierungen

> **📌 Minimal Local Agent:** Alle Schlüssel werden lokal im Dateisystem gespeichert (`keys/` Verzeichnis). Hardware-Sicherheitsmodule (HSM/PKCS#11) und Cloud-Dienste (CloudKMS) sind nicht verfügbar.

**Schlüsselformat (cap-key.v1):**
- `*.ed25519` – Private Key (32 bytes)
- `*.pub` – Public Key (32 bytes)
- `*.v1.json` – Key-Metadaten (KID, Owner, Gültigkeit)
...
**Verwandte Begriffe:** [Digital Signature](#digital-signature), [Public Key](#public-key), [Private Key](#private-key), [KID](#kid-key-id)
...
**Im Projekt:** Proof-Backend als Plugin (Mock) via Trait-Implementierung.
...
- Mock (einziges verfügbares Backend in Minimal Local Agent)
- ~~Halo2 (entfernt in v0.12.0)~~
- ~~Spartan (entfernt in v0.12.0)~~

**Verwandte Begriffe:** [Prover](#prover), [Verifier](#verifier)

> **📌 Minimal Local Agent:** Nur das Mock-Backend ist verfügbar. ZK-Backends (zkvm, halo2, spartan) wurden in dieser Version entfernt.
...
**Im Projekt (Minimal Local Agent):** Das **einzige verfügbare Backend** in der Minimal Local Agent Version. Erzeugt strukturierte Proofs gegen Policy-Constraints ohne kryptographische Garantien.

**Vorteil:** Schnell, einfach zu debuggen, keine externen Abhängigkeiten.
...
**Verwandte Begriffe:** [Zero-Knowledge Proof](#zero-knowledge-proof-zkp)
...
## 17. WebUI & Frontend (Tauri-integriert ab v0.12.0)
...
**Im Projekt (v0.12.2):**
- Version: React 19.2
- Components: BundleUploader, ManifestViewer, VerificationView, WorkflowStepper, SigningView, AuditTimeline
- State Management: Zustand Stores (workflowStore, verificationStore)
...
- Type Definitions: Alle Tauri IPC types definiert (ProjectInfo, ManifestResult, etc.)
...
**Im Projekt (v0.12.2):**
- Version: Vite 7.2
- Dev Server: Läuft innerhalb Tauri Dev
...
**Im Projekt (v0.12.2):**
- **BundleUploader:** Tauri-File-Dialog-basierter Upload
- **ManifestViewer:** Zeigt Manifest-Daten an
- **VerificationView:** Zeigt Verification Results
- **WorkflowStepper:** 6-Schritte Proofer-Workflow
- **SigningView:** Ed25519 Manifest-Signierung
- **AuditTimeline:** Audit-Log Visualisierung
...
- **useEffect:** Side Effects (z.B. Tauri IPC Calls)
- **useCallback:** Memoization für stabile Funktionsreferenzen
...
const loadManifest = useCallback(async () => {
  const result = await invoke<ManifestResult>('build_manifest', { project: projectPath });
  setManifest(result);
}, [projectPath]);
...
**Im Projekt (v0.12.2):**
- React-basiertes Frontend in Tauri WebView ist eine SPA
- Keine Page Reloads bei Navigation
- Tab-basierte Navigation (Prover/Verifier Modes)
...
- Desktop-optimiert (Tauri Window)
- Responsive Grid Layouts innerhalb der App
...
H: [Hash](#hash--hash-funktion), [Hash Chain](#hash-chain), [Health Check](#health-check), [Hexadecimal](#hexadecimal-hex), [Horizontal Scaling](#horizontal-scaling), [HTTP](#http-hypertext-transfer-protocol), [HTTPS](#https-http-secure)
...
**Im Projekt:** Alle Tauri-Aufrufe in `tauri-frontend/src/lib/tauri.ts` nutzen `invoke()`.
...
**Im Projekt:** `tauri-frontend/src/components/workflow/WorkflowStepper.tsx`
...
**Im Projekt:** `tauri-frontend/src/components/layout/ProjectSidebar.tsx`
...
**Im Projekt:** `tauri-frontend/src/components/audit/AuditTimeline.tsx`
...
*Projekt: LsKG-Agent v0.12.2*

**Changelog v2.4:**
- 🧹 Entfernte Features bereinigt (Axios, Bearer Token, Halo2 Dokumentation gelöscht)
- 📊 Gesamt: 210+ Begriffe (nur aktive Features)

**Changelog v2.3:**
- 🔄 Kategorie 17: Aktualisiert für Tauri-Integration (React 19.2, Vite 7.2, TailwindCSS 4.x)
- 🔄 Komponenten-Liste erweitert: SigningView, AuditTimeline, WorkflowStepper
- 🔄 Hooks: useCallback hinzugefügt
