...
| Metrik | Wert | Details |
|--------|------|---------|
| **Enterprise Readiness** | 57% | Aktuell |
| **Ziel nach Hardening** | 95% | 14 Wochen Roadmap |
| **Kritische Findings** | 4 | F-001 bis F-004 |
| **Audit-Dokument** | [SECURITY_AUDIT_REPORT.md](../../security/SECURITY_AUDIT_REPORT.md) | Vollständiger Report |

> ⚠️ **Hinweis:** Viele der unten beschriebenen Probleme können durch Enterprise Security Hardening vermieden werden. Siehe [Enterprise Security Troubleshooting](#-enterprise-security-troubleshooting-neu---dezember-2025) am Ende dieses Dokuments.
...
## 🔐 Enterprise Security Troubleshooting (NEU - Dezember 2025)

Dieser Abschnitt behandelt sicherheitsrelevante Probleme, die im Rahmen des Enterprise Security Audits identifiziert wurden.

### 17. CORS Security Issues (F-001)

> ⚠️ **Kritisches Finding:** CORS Allow-All Konfiguration in Production

**Symptom:**
```
Browser Console: Access-Control-Allow-Origin: *
Security Scanner: CORS Misconfiguration detected
```

**Ursache:** Standard-Konfiguration mit `allow_origin(Any)` ist für Production nicht sicher.

**Aktuelle Konfiguration (UNSICHER):**
```rust
// agent/src/bin/verifier_api.rs
let cors = CorsLayer::new()
    .allow_origin(Any)  // ❌ NICHT FÜR PRODUCTION!
    .allow_methods([Method::GET, Method::POST, Method::OPTIONS])
    .allow_headers(Any);
```
...
// Explizite Origin-Liste für Production
let allowed_origins = [
    "https://cap-verifier.ihrefirma.de",
    "https://admin.ihrefirma.de",
];
...
let cors = CorsLayer::new()
    ...
    .allow_methods([Method::GET, Method::POST, Method::OPTIONS])
    .allow_headers([AUTHORIZATION, CONTENT_TYPE])
    .allow_credentials(true)  // Nur wenn nötig
...
```bash
# Production
export ALLOWED_ORIGINS="https://cap-verifier.ihrefirma.de,https://admin.ihrefirma.de"
...
```bash
# CORS-Header prüfen
curl -I -X OPTIONS http://localhost:8080/verify \
  -H "Origin: https://evil-site.com" \
  -H "Access-Control-Request-Method: POST"

# Erwartetes Ergebnis: Keine Access-Control-Allow-Origin Header für fremde Origins
```

---

### 18. Missing Security Headers (F-002)

> ⚠️ **Kritisches Finding:** Fehlende HTTP Security Headers

**Symptom:**
```
Security Scanner: Missing HSTS header
Security Scanner: Missing X-Frame-Options header
Security Scanner: Missing Content-Security-Policy header
```

**Fehlende Header:**
| Header | Zweck | Status |
|--------|-------|--------|
| `Strict-Transport-Security` | HTTPS erzwingen | 🔴 Fehlt |
| `X-Frame-Options` | Clickjacking-Schutz | 🔴 Fehlt |
| `X-Content-Type-Options` | MIME-Sniffing verhindern | 🔴 Fehlt |
| `Content-Security-Policy` | XSS-Schutz | 🔴 Fehlt |
| `X-XSS-Protection` | Legacy XSS-Filter | 🔴 Fehlt |
| `Referrer-Policy` | Referrer-Kontrolle | 🔴 Fehlt |

**Lösung - Tower-HTTP Security Layer:**
```rust
...
curl -I https://localhost:8443/healthz
...
**Security Header Check Tools:**
- [SecurityHeaders.com](https://securityheaders.com)
- [Mozilla Observatory](https://observatory.mozilla.org)

---

### 19. Hardcoded Mock Keys (F-003)

> ⚠️ **Kritisches Finding:** Hardcodierte Test-Keys im Code

**Symptom:**
```
Code Review: RSA keys hardcoded in auth.rs
Security Scanner: Hardcoded credentials detected
```

**Betroffene Datei:** `agent/src/api/auth.rs:50-100`

**Problematischer Code:**
```rust
// ❌ NIEMALS in Production verwenden!
const MOCK_RSA_PRIVATE_KEY: &str = "-----BEGIN PRIVATE KEY-----
MIIEvgIBADANBgkqhkiG9w0BAQE...
-----END PRIVATE KEY-----";

const MOCK_RSA_PUBLIC_KEY: &str = "-----BEGIN PUBLIC KEY-----
MIIBIjANBgkqhkiG9w0BAQE...
-----END PUBLIC KEY-----";
```

**Lösung - Externe Key-Konfiguration:**
```rust
// Keys aus Environment oder Dateien laden
pub fn load_oauth2_config() -> Result<OAuth2Config> {
    let public_key_path = std::env::var("OAUTH2_PUBLIC_KEY_PATH")
        .unwrap_or_else(|_| "/etc/cap-verifier/oauth2-public.pem".to_string());

    let public_key = std::fs::read_to_string(&public_key_path)
        .context(format!("Failed to read public key from {}", public_key_path))?;

    Ok(OAuth2Config {
        issuer: std::env::var("OAUTH2_ISSUER")
            .unwrap_or_else(|_| "https://auth.example.com".to_string()),
        audience: std::env::var("OAUTH2_AUDIENCE")
            .unwrap_or_else(|_| "cap-verifier".to_string()),
        public_key,
...
**Kubernetes Secret für Production:**
```yaml
apiVersion: v1
kind: Secret
metadata:
  name: cap-verifier-oauth2
type: Opaque
stringData:
  public-key.pem: |
    -----BEGIN PUBLIC KEY-----
  ...
  template:
    spec:
      ...
      volumes:
        - name: oauth2-secrets
          secret:
            secretName: cap-verifier-oauth2
```

**Verifikation:**
```bash
# Prüfen ob Mock-Keys im Code sind
grep -r "BEGIN PRIVATE KEY" agent/src/
grep -r "BEGIN RSA PRIVATE KEY" agent/src/

# Sollte KEINE Treffer in Production-Code liefern
```

---

### 20. Dev-Token Bypass (F-004)

> ⚠️ **Kritisches Finding:** Admin-Token "admin-tom" bypassed Authentication

**Symptom:**
```
# Beliebiger Zugriff möglich mit:
curl -H "Authorization: Bearer admin-tom" http://localhost:8080/verify
```

**Betroffene Datei:** `agent/src/api/auth.rs:34-45`

**Problematischer Code:**
```rust
// ❌ NIEMALS in Production!
pub fn validate_token(token: &str, config: &OAuth2Config) -> Result<Claims> {
    // DEV ONLY: Accept simple admin token
    if token == "admin-tom" {
        return Ok(Claims {
            sub: "admin".to_string(),
...
**Lösung - Compile-Time Feature Flag:**
```rust
// In auth.rs
pub fn validate_token(token: &str, config: &OAuth2Config) -> Result<Claims> {
    ...
    {
        if token == "admin-tom" && std::env::var("ALLOW_DEV_TOKEN").is_ok() {
            tracing::warn!("⚠️ DEV TOKEN USED - NOT FOR PRODUCTION!");
            return Ok(Claims {
                sub: "admin-dev".to_string(),
                // ...
            });
        }
    }

    // Production: Normale JWT-Validierung
...
[features]
default = []
dev-auth = []  # Nur für Entwicklung aktivieren

# In auth.rs:
#[cfg(feature = "dev-auth")]
fn accept_dev_token() -> bool { true }

#[cfg(not(feature = "dev-auth"))]
fn accept_dev_token() -> bool { false }
```

**Production Build (ohne Dev-Token):**
```bash
# Release Build hat KEINE Dev-Token Unterstützung
cargo build --release --bin cap-verifier-api

# Development Build (mit Dev-Token)
cargo build --features dev-auth --bin cap-verifier-api
```

**Verifikation:**
```bash
# Test: Dev-Token sollte in Release-Build NICHT funktionieren
curl -I -H "Authorization: Bearer admin-tom" http://localhost:8080/verify
# Erwartetes Ergebnis: 401 Unauthorized
```

---

### 21. OAuth2 JWKS Integration Issues

**Symptom:**
```
Error: Failed to validate JWT - Key not found
Error: JWKS endpoint unreachable
Warning: Using static public key (no rotation)
```

**Ursache:** JWKS (JSON Web Key Set) für automatische Key-Rotation nicht implementiert.

**Aktuelle Einschränkung:**
- Statischer Public Key in PEM-Format
- Keine automatische Key-Rotation
- Manueller Key-Update erforderlich

**Lösung - JWKS-Integration (geplant für Phase 2):**
```rust
...
impl JwksCache {
    pub async fn new(jwks_url: &str) -> Result<Self> {
        let keys = fetch_jwks(jwks_url).await?;
        let cache = Self {
            keys: Arc::new(RwLock::new(keys)),
            jwks_url: jwks_url.to_string(),
            refresh_interval: Duration::from_secs(3600), // 1 Stunde
        };

        // Background-Refresh starten
...
**Workaround bis JWKS-Integration:**
```bash
# Manueller Key-Update via Kubernetes Secret
kubectl create secret generic cap-verifier-oauth2 \
  --from-file=public-key.pem=new-public-key.pem \
  --dry-run=client -o yaml | kubectl apply -f -

# Pod-Restart für Key-Reload
kubectl rollout restart deployment/cap-verifier-api
```

---

### 22. Rate Limiting Bypass Attempts

**Symptom:**
```
Logs: Multiple 429 responses from same client
Logs: Distributed attack from multiple IPs
Metrics: Rate limit exceeded spike
```

**Aktuelle Einschränkung:**
- IP-basiertes Rate Limiting
- Kein User-basiertes Limiting
- Keine Distributed Rate Limiting (Multi-Instance)
...
# Logs nach Rate Limit Events durchsuchen
journalctl -u cap-verifier | grep "429"
docker logs cap-verifier 2>&1 | grep "rate"
```

**Lösung für Distributed Setup:**
```bash
# Nginx/HAProxy Sticky Sessions
upstream cap-backend {
    ip_hash;  # Gleiche IP → gleiches Backend
    server backend1:8080;
    server backend2:8080;
}

# Oder: Redis-basiertes Rate Limiting (geplant)
# Shared State zwischen allen Instanzen
```

---

### 23. TLS Certificate Problems
...
**A) Self-Signed Certificate nicht trusted:**
```bash
# Symptom
...
# NICHT für Production! Stattdessen:
# 1. Let's Encrypt Certificate verwenden
# 2. Oder CA-Certificate zum Trust Store hinzufügen
```

**B) Certificate Chain incomplete:**
```bash
# Chain prüfen
openssl s_client -connect localhost:8443 -showcerts

# Vollständige Chain in server.crt:
# 1. Server Certificate
# 2. Intermediate CA (falls vorhanden)
cat server.crt intermediate.crt > fullchain.crt
```

**C) Certificate abgelaufen:**
```bash
# Ablaufdatum prüfen
openssl x509 -in /certs/server.crt -noout -enddate

# Automatische Renewal (Let's Encrypt)
certbot renew --deploy-hook "systemctl reload cap-verifier"
```

**D) mTLS Client Certificate fehlt:**
```bash
# Symptom
curl: (35) error:1401E412:SSL routines:CONNECT_CR_FINISHED:sslv3 alert handshake failure

# Lösung: Client Certificate mitgeben
curl --cert client.crt --key client.key https://localhost:8443/verify
```

---

### 24. Graceful Shutdown Issues

**Symptom:**
```
Warning: Requests dropped during deployment
Error: Connection reset by peer
Logs: SIGTERM received, immediate exit
```

**Ursache:** Kein Graceful Shutdown implementiert (Security Score: 0%)

**Aktuelle Situation:**
- Sofortiger Exit bei SIGTERM
- Laufende Requests werden abgebrochen
- Keine Drain-Phase

**Lösung (geplant für Phase 3):**
```rust
use tokio::signal;
...
// Kubernetes: Readiness Probe deaktivieren vor Shutdown
// preStop hook: sleep 5 (Drain-Phase)
```
...
### Enterprise Security Checkliste

**Vor Production-Deployment prüfen:**

```
□ F-001: CORS auf explizite Origins konfiguriert
□ F-002: Alle Security Headers aktiv
□ F-003: Keine hardcodierten Keys im Code
□ F-004: Dev-Token in Release-Build deaktiviert
□ TLS/mTLS aktiv und Certificates gültig
□ OAuth2 mit echtem Identity Provider
...
docker run -t owasp/zap2docker-stable zap-baseline.py \
  -t https://cap-verifier.example.com
...
*Dokument-Version: 2.1 (aktualisiert mit Enterprise Security Troubleshooting)*
